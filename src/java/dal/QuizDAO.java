    ...
    public ArrayList<Ques_Ans> getQuestion_AnswerList(String search, int accountID) {
        ...
        try {
            ...
            if (search.equals("wrong")) {
                sql += "and where answer_id <> correct_answer and answer_id is not null";
            } else if (search.equals("correct")) {
                sql += "and where answer_id = correct_answer";
            } else if (search.equals("none")) {
                sql += "and where answer_id is null";
            }
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, accountID);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                ...
                qa.setQuestion(qdao.getQuestionById(rs.getInt(1)));
