...
//if Back Que button clicked
    back_btn.onclick = () => {
        if (que_count > 0) { //if question count is less than total question length
            ...
            back_btn.classList.remove("show"); //hide the next button
        }
        if (que_count == 0) {
...
//        clearInterval(counterLine); //clear counterLine
            next_btn.classList.remove("show"); //hide the next button
            submit_btn.classList.add("show"); //hide the next button
...
//    clearInterval(counterLine); //clear counterLine
        ...
        for (i = 0; i < allOptions; i++) {
            if (option_list.children[i] != answer)
                option_list.children[i].classList.remove("selected"); //once user select an option then disabled all options
        }
...
//            next_btn.classList.add("show"); //show the next button if user selected any option
    ...
    function startTimerLine(time) {
        ...
        function timer() {
            ...
            if (time > window.innerWidth) { //if time value is greater than 549
                clearInterval(counterLine); //clear counterLine
            }
